...
- `download:account` lists the account library through `/api/feed/v3`, retrieves ordinary MP3/WAV through encrypted playback media, decrypts locally, converts with FFmpeg, and records resumable local state in `manifest.json`. These files are for personal use and must not be used for commercial monetization; see the account-archive guide for the C2PA and official-authorization limitations.
