...
MP3 和 WAV 都从当前播放媒体生成：读取 `media_urls`，对加密流获取 Mango rights 并本地解密，再由本地 FFmpeg 转换。不会请求 Suno 的官方下载、`convert_wav` 或 `wav_file`。

WAV 是播放源解码后的 PCM，不是官方原始无损母带；播放音频失败时会记录该格式错误，后续重新执行相同归档命令即可重试，不会自动切换到官方下载。FFmpeg 是必需依赖。
